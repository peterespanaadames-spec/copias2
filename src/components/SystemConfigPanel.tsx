...
interface SystemConfigPanelProps {
  ...
  initialSubTab?: 'mi_cuenta' | 'mi_negocio' | 'facturacion' | 'inventario' | 'impresion' | 'dashboard' | 'notificaciones' | 'planes_suscripcion' | 'mantenimiento';
}
...
export const SystemConfigPanel: React.FC<SystemConfigPanelProps> = ({
  ...
  const [configSubTab, setConfigSubTab] = useState<'mi_cuenta' | 'mi_negocio' | 'facturacion' | 'inventario' | 'impresion' | 'dashboard' | 'notificaciones' | 'planes_suscripcion' | 'mantenimiento'>(initialSubTab || 'mi_negocio');
    ...
    <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-xs text-left" id="general_system_configuration_center">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-gray-200 pb-5 mb-6 font-poppins">
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          {/* 💾 BOTÓN DE DESCARGA RÁPIDA DE RESPALDO */}
          <button
            type="button"
            onClick={handleDownloadFullBackup}
            disabled={isExportingBackup}
            className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-montserrat text-xs font-extrabold rounded-xl flex items-center gap-2 transition shadow-md hover:shadow-emerald-600/30 cursor-pointer border-b-2 border-emerald-400 disabled:opacity-50"
          ...
          >
            ...
            <span>{isExportingBackup ? 'Generando Respaldo...' : 'Descargar Respaldo (.JSON)'}</span>
          </button>
