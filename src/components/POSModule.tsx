...
export default function POSModule({ 
  ...
  const taxItemsSummary = useMemo(() => {
    const rateForVes = customBcvRate > 0 ? customBcvRate : (currencyRates.VES || bcvRate || 45.5);
  ...
  const handlePostponeSale = async () => {
    ...
    try {
      ...
      // Limpia todos los formularios y vuelve a la venta directa
      resetVentaFlash();
      
      showToast('success', `¡Factura guardada en espera exitosamente (${calculatedDraftRef})! Formulario restablecido para venta directa.`);
    } catch (err: any) {
  ...
  const handleWizardNextStep1 = async () => {
    ...
    if (wizardIsNewClient) {
      if (!wizardSelectedClientObj.name?.trim()) {
        showToast('error', 'El nombre es obligatorio para registrar un nuevo cliente.');
        return;
      }
      if (!wizardSelectedClientObj.document?.trim()) {
      ...
      setIsLoading(true);
      try {
        const docUpper = wizardSelectedClientObj.document.trim().toUpperCase();
        const clientPayload = {
          name: wizardSelectedClientObj.name.trim(),
          document: wizardSelectedClientObj.document.trim(),
        ...
        if (created) {
          ...
          // Add to clients state so they appear in lists
          setClients(prev => [created, ...prev]);
          setWizardSelectedClientObj(created);
          setSelectedClient(created.name);
      ...
      // If user filled in or edited document, phone, email, or address, update client record
      if (wizardSelectedClientObj.id && !String(wizardSelectedClientObj.id).startsWith('cf')) {
        try {
          dbService.updateClient(wizardSelectedClientObj.id, {
            name: wizardSelectedClientObj.name,
            document: wizardSelectedClientObj.document,
            phone: wizardSelectedClientObj.phone,
            email: wizardSelectedClientObj.email,
            address: wizardSelectedClientObj.address
          }).catch(e => console.warn('Non-blocking client update notice:', e));

          setClients(prev => prev.map(c => c.id === wizardSelectedClientObj.id ? { ...c, ...wizardSelectedClientObj } : c));
        } catch (e) {
          console.warn('Error updating client in wizard:', e);
        }
      }
      setSelectedClient(wizardSelectedClientObj.name);
      setWizardStep(2);
    }
  };
    ...
    <div className="relative text-left p-4 md:p-6 bg-[#F8F9FA] min-h-screen font-poppins text-[#2B2D42]">
      ...
      <div className="bg-white border border-[#1D3557]/15 rounded-2xl shadow-xs p-4 mb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button 
              ...
              className={`px-3.5 py-2 text-xs font-montserrat font-extrabold rounded-xl transition-all flex items-center gap-2 shadow-md cursor-pointer border ${
                draftInvoices.length > 0
                  ? 'bg-[#005da9] hover:bg-[#004b88] text-white border-[#005da9] animate-pulse hover:shadow-lg'
            ...
            >
              <Pause className={`w-4 h-4 shrink-0 ${draftInvoices.length > 0 ? 'fill-white' : 'fill-[#005da9]'}`} />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <div className="xl:col-span-5 space-y-6">
          ...
          <div className="bg-white border border-[#1D3557]/15 rounded-2xl shadow-xs p-5">
            {/* Selector de Tipo de Documento: Factura vs Nota de Entrega */}
            <div className="mb-3.5 p-1 bg-gray-100/80 rounded-2xl flex items-center border border-gray-200/60 shadow-3xs">
              <button
                type="button"
                onClick={() => {
                  setDocumentType('factura');
                ...
                className={`flex-1 py-2 px-3 text-xs font-montserrat font-extrabold rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer ${
                  documentType === 'factura'
                    ? 'bg-[#1D3557] text-white shadow-xs'
                    : 'text-gray-600 hover:text-[#1D3557]'
                }`}
              >
                <FileText className={`w-4 h-4 ${documentType === 'factura' ? 'text-[#40E0D0]' : 'text-gray-400'}`} />
                <span>Factura</span>
              </button>
              <button
                type="button"
              ...
              >
                <FileCheck className={`w-4 h-4 ${documentType === 'nota_entrega' ? 'text-white' : 'text-gray-400'}`} />
              ...
              <div className="mt-5 pt-4 border-t border-gray-100 space-y-4">
                ...
                <div>
                  ...
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full py-3 bg-[#40E0D0] hover:bg-[#36cebe] text-[#1D3557] font-montserrat font-black text-xs uppercase tracking-widest rounded-xl transition shadow-md hover:shadow-lg flex items-center justify-center gap-2 cursor-pointer border-b-2 border-[#1D3557]/20"
                  >
                    <span>Finalizar Venta</span>
                    <ChevronRight className="w-4 h-4 text-[#1D3557] stroke-[3]" />
                  </button>
                </div>
              </div>
            )}
                    ...
                    <div className="flex justify-between items-center text-xs text-amber-900 font-semibold">
                      ...
                      <div className="text-right">
                        ...
                        <span className="font-mono text-amber-950 text-xs sm:text-sm font-black ml-1.5">
                          (Bs. {((applyIgtf ? calculatedIgtfUsd : 0) * (customBcvRate > 0 ? customBcvRate : (currencyRates.VES || bcvRate || 45.5))).toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                        </span>
                      </div>
                    </div>
                  )}
        ...
        {showCheckoutWizard && (
          <div className="w-full max-w-6xl mx-auto font-montserrat mt-4">
            ...
            >
              ...
              <div className="p-5 md:px-8 md:pt-6 pb-2 flex items-center justify-between border-b border-gray-100 bg-white">
                <button
                  type="button"
              ...
              <div className="p-6 md:p-8 overflow-y-auto flex-1 max-h-[65vh]">
                ...
                {wizardStep === 1 && (
                  <div className="space-y-5 max-w-2xl mx-auto">
                    {/* Selector Tipo de Comprobante en Step 1 */}
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3.5 bg-gray-50/80 border border-gray-200/80 rounded-2xl">
                      <div>
                        <h2 className="text-sm font-black text-[#1D3557] uppercase tracking-tight">
                          Datos del cliente
                        </h2>
                        <p className="text-[10px] text-gray-500 font-medium">
                          Comprobante actual: <strong className="text-[#1D3557]">{documentType === 'factura' ? 'Factura de Venta' : 'Nota de Entrega'}</strong>
                        </p>
                      </div>

                      <div className="flex bg-white p-1 rounded-xl border border-gray-200 shadow-3xs">
                        <button
                          type="button"
                          onClick={() => {
                            setDocumentType('factura');
                          ...
                          className={`py-1.5 px-3 text-xs font-montserrat font-extrabold rounded-lg transition-all flex items-center gap-1.5 cursor-pointer ${
                            documentType === 'factura'
                              ? 'bg-[#1D3557] text-white shadow-xs'
                              : 'text-gray-500 hover:text-gray-800'
                          }`}
                        >
                          <FileText className={`w-3.5 h-3.5 ${documentType === 'factura' ? 'text-[#40E0D0]' : 'text-gray-400'}`} />
                          <span>Factura</span>
                        </button>
                        <button
                          type="button"
                        ...
                        >
                          <FileCheck className={`w-3.5 h-3.5 ${documentType === 'nota_entrega' ? 'text-white' : 'text-gray-400'}`} />
                          <span>Nota de Entrega</span>
                        </button>
                      </div>
                    </div>
                ...
                {wizardStep === 2 && (
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start max-w-6xl mx-auto">
                    ...
                    <div className="lg:col-span-7 space-y-4">
                      ...
                      {/* 🎛️ BUTTONS: AL CONTADO | CUENTAS POR COBRAR | FACTURA EN ESPERA */}
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <button
                          ...
                          disabled={isLoading || cart.length === 0}
                          className="group py-3.5 px-4 font-extrabold text-xs uppercase tracking-wider rounded-2xl border border-gray-200 bg-white text-gray-700 hover:bg-[#005da9] hover:text-white hover:border-[#005da9] active:bg-[#004b88] active:text-white active:border-[#004b88] transition-all duration-300 flex items-center justify-center gap-2 shadow-2xs cursor-pointer active:scale-95 disabled:opacity-50"
                          title="Colocar esta factura en espera, limpiar formularios y volver a la venta directa"
                        >
                          <Pause className="w-4 h-4 text-[#005da9] group-hover:text-white group-active:text-white transition-colors shrink-0" />
                          <span className="truncate">Colocar en espera</span>
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-end bg-white border border-gray-100 p-3.5 rounded-2xl shadow-3xs">
                        <div className="flex items-center gap-2 bg-[#005da9]/5 border border-[#005da9]/20 px-3.5 py-1.5 rounded-full text-xs">
                          ...
                          <input
                            ...
                            step="0.01"
                            min="0"
                            value={customBcvRate > 0 ? customBcvRate : (bcvRate ? Number(bcvRate.toFixed(2)) : 45.50)}
                            onChange={(e) => {
                        ...
                        <>
                          ...
                          <div className="space-y-3.5">
                            {splitPayments.map((p, idx) => {
                              ...
                              const effectiveRate = customBcvRate > 0 ? customBcvRate : (currencyRates.VES || bcvRate || 45.5);
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start max-w-5xl mx-auto">
                    ...
                    <div className="lg:col-span-7 space-y-4">
                      <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-2xs space-y-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            <label className="block text-[11px] font-black uppercase text-gray-400 mb-1">Fecha</label>
                            <input
                              type="date"
                              value={saleDate}
                              onChange={(e) => setSaleDate(e.target.value)}
                              className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-xs font-mono font-bold text-gray-700 focus:outline-none focus:ring-2 focus:ring-[#005da9]"
                            />
                          </div>
                          ...
                          <div>
                              ...
                              <div className="flex gap-1 text-[10px] font-black">
                                <button
                                  ...
                                  className={`px-2 py-0.5 rounded cursor-pointer transition ${
                                    documentType === 'factura' ? 'bg-[#1D3557] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                                  }`}
                                >
                                  Factura
                                </button>
                                <button
                                  ...
                                  className={`px-2 py-0.5 rounded cursor-pointer transition ${
                                    documentType === 'nota_entrega' ? 'bg-[#005da9] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                                  }`}
                                >
                                  Nota
                                </button>
                            ...
                            <input
                              ...
                              className="w-full px-3 py-2 bg-gray-100 border border-gray-200 rounded-xl text-xs font-mono font-black text-[#004b88]"
                            />
                          </div>
                        </div>
